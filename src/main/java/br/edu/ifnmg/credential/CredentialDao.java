...
import br.edu.ifnmg.gui.CadastroUsuario;
import br.edu.ifnmg.gui.TelaPrincipal;
...
public class CredentialDao extends Dao<Credential> {
    ...
    public String getSaveStatement() {
        return "insert into " + TABLE + " (username, password, last_access, enabled, id) values(?, ?, ?, ?, ?)";
    }
...
//    }
    ...
    @Override
    public void composeSaveOrUpdateStatement(PreparedStatement pstmt, Credential e) {
        if (TelaPrincipal.current_cred == null) {
            try {
                if (e.getId() != null && e.getId() < 0) {
                    pstmt.setString(1, e.getUsername());
                    pstmt.setString(2, md5(e.getPassword() + SALT));
